...
export class UserController {
  ...
  public async getAllUsers(@request() req: Request, @response() res: Response) {
    try {
      const result: Array<User> = await this._userService.getAll(
  ...
  public async getUserById(
    ...
    try {
      const result: User | undefined = await this._userService.getById(
        req.params.user_id,
        new Query().fromJSON(req.query)
      )
      if (!result)
  ...
  public async removeUser(
    ...
    try {
      const result: boolean = await this._userService.remove(
        req.params.user_id
      )
      if (!result)
        return res
  ...
  private toJSONView(entity: User | Array<User> | undefined): object {
    if (entity instanceof Array)
      return entity.map((item) => this.toJSONView(item))
